...
export class AiController {
  ...
  async generateArtifact(
    @Req() request: Request,
    @Param("projectId") projectId: string,
    @Body() body: { conversationId?: string; clientRequestId?: string; type?: string; instruction?: string; provider?: string; density?: string; customProvider?: unknown }
  ) {
    const user = await this.auth.requireUser(request);
  ...
  async generateDeck(
    @Req() request: Request,
    @Param("projectId") projectId: string,
    @Body() body: { conversationId?: string; clientRequestId?: string; planArtifactId?: string; provider?: string; density?: string; customProvider?: unknown }
  ) {
    const user = await this.auth.requireUser(request);
